...
class Runner(object):
    ...
    """
    compute()
      → policy.get_values()      # 获取最后状态的价值
      → buffer.compute_returns() # 计算GAE和returns
    """
    @torch.no_grad()
    def compute(self):
        """Calculate returns for the collected data.作用: 计算优势函数和目标价值"""
        ...
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            ...
            next_values = self.trainer.policy.get_values(np.concatenate(self.buffer.share_obs[-1]),     #  获取最后状态的价值
                                                        np.concatenate(self.buffer.rnn_states_critic[-1]),
                                                        np.concatenate(self.buffer.masks[-1]))
        next_values = np.array(np.split(_t2n(next_values), self.n_rollout_threads))
        self.buffer.compute_returns(next_values, self.trainer.value_normalizer)     # 计算GAE和returns

    """
    train()
      → buffer.feed_forward_generator()  # 生成训练批次
      → policy.ppo_update()              # PPO更新
      → trainer.train()                  # 执行梯度更新
    """
    def train(self):
