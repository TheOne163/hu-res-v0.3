...
"""
def main(args):
    parser = get_config()       #第一步，解析参数
    all_args = parse_args(args, parser)     #第二步，获取配置

    if all_args.algorithm_name == "rmappo":
        print("u are choosing to use rmappo, we set use_recurrent_policy to be True")

        #启用基于 RNN（如 GRU/LSTM）的策略网络，使 actor/critic 能够携带时间记忆。
        # 输入由obs → obs + hidden_state
        # 输出包括action + new_hidden_state
        #naive recurrent policy（朴素版 RNN）naive RNN 在 MARL 中基本不推荐。
        ...
        all_args.use_recurrent_policy = False       #不启用基于 RNN（如 GRU/LSTM）的策略网络，与rmappo的区别
        all_args.use_naive_recurrent_policy = False
    ...
    if all_args.use_wandb:
        ...
        ## 将 wandb.config 转换为普通字典，以便多进程传递时不出问题
        # all_args = types.SimpleNamespace(**wandb.config)
    ...
    if all_args.share_policy:
        from onpolicy.runner.shared.smac_runner import SMACRunner as Runner     #所有智能体共享一个 MAPPO/RMAPPO 策略,ramppo和mappo默认true
    else:
        from onpolicy.runner.separated.smac_runner import SMACRunner as Runner      #每个智能体都有独立策略
